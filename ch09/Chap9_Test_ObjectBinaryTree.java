...
class SimpleObject4 {
	...
	public SimpleObject4(int no, String name) {
		this.no = no;
		this.name = name;
	}
...
class Tree4 {
	...
	private int getDepth(TreeNode4 current_node) {
		if (current_node == null) return 0;
        int lDepth = getDepth(current_node.LeftChild);
        int rDepth = getDepth(current_node.RightChild);
	...
	public boolean add(SimpleObject4 obj, Comparator<? super SimpleObject4> c) {
		...
		TreeNode4 current_node = root, parent_node = null;
		...
		while (current_node != null) {
			if (c.compare(current_node.data, tmp.data) == 0)
				return false;
		...
		if (c.compare(parent_node.data, tmp.data) < 0)
			parent_node.RightChild = tmp;

		else
			parent_node.LeftChild = tmp;

		return true;

	}

	void delete_cal(TreeNode4 current_node, TreeNode4 parent_node, int branchMode) {
		if (isLeafNode(current_node)) {
			...
			if (current_node.LeftChild == null) {
				...
				TreeNode4 tmp = new TreeNode4();
				tmp = inorderSucc(current_node);
				delete(tmp.data, SimpleObject4.NO_ORDER);
	...
	public boolean delete(SimpleObject4 obj, Comparator<? super SimpleObject4> c) {
		...
		while (current_node != null) {
			if (c.compare(current_node.data, obj) == 0) {
	...
	// root 부터 level별로 출력 : root는 level 1, level 2는 다음줄에 => 같은 level이면 같은 줄에 출력하게 한다
	{
	    ...
	    ObjectQueue4 q = new ObjectQueue4(10000);
	    q.enque(root);
	    int depth = getDepth();
	    ...
	    while (!q.isEmpty()) {
	        ...
	        int count = q.size();
	        System.out.print("        ".repeat(Math.max((int) Math.pow(2, (depth - current_depth - 1)) - 1, 0)));

	        while (count-- > 0) {
	            TreeNode4 currentNode = q.deque();
	            ...
	            System.out.print("        ".repeat(Math.max((int) Math.pow(2, (depth - current_depth)) - 1, 0)));
...
public class Chap9_Test_ObjectBinaryTree {
